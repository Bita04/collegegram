...
import { addComment, getComments } from "../../api/appApi";
...
function PostDetail({ likes, saves, tags, id, description }: Props) {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) =>
    setCommentsPost([...commentsPost, data.comment]);
  ...
  const {
    ...
    ({ pageParam }) => {
      ...
      return getComments(+id!, 3, pageParam, first)
    },
    {
  ...
  return (
    <>
      <Flex flexDirection={"column"} className="mr-[39px]">
        ...
        <span className="my-[24px] color-secondary text-sm/[11px]">
          ۵ ماه پیش
        </span>
        <p className="w-[462px] color-secondary mb-[16px] leading-[32px]">
          {description}{" "}
        </p>
        <Tags tags={tags} />
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="overflow-hidden">
          ...
          >
            ...
            <Input
              ...
              className="  placeholder-[#CDCDCD]"
              placeholder="نظر خود را بنویسید"
            />
            <button onClick={handleClick}>
              <Image w={"24px"} src={send} />
            </button>
          </Stack>
        </form>
        ...
        <InfiniteScroll
          ...
          next={() =>
            fetchNextPage({
              pageParam: comments?.pages[comments.pages.length - 1].nextOffset,
            })
          }
          hasMore={hasMore}
          loader={<p>Loading...</p>}
